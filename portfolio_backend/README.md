...
Backend Express 5 en TypeScript pour le portfolio d'Elyas Benyoub.

Points à préserver :

- API projets avec champs CMS : `image_url`, `gallery_images`, `display_settings`, `github_url`, `demo_url`.
- Carousel accueil : `is_featured` et `featured_order`.
- Uploads stockés côté backend dans le volume Docker `uploads_data`.
- Schéma de référence pour base fraîche : `../db/init.sql`.
- Middleware de migration légère : `src/config/schema.ts`.
...
npm run lint
npm run build
npm run github:import -- --account elyas-benyoub --cache-dir /tmp/portfolio-project-audit --apply
```
...
Les images uploadées depuis l'admin sont stockées côté backend dans `uploads/projects/` et servies via l'URL publique `/uploads/projects/<nom-du-fichier>`.
Avec Docker, le volume `uploads_data` conserve ces fichiers même si le conteneur backend est recréé.
